...
def add_book(library):
    ...
    def search_library(library):
        search_by = input('Search by tutle or author').lower()
        ...
        if results :
            for book in results:
                status = "Read" if book['read'] else "undread"
                print(f'{book['title']} by {book['author']} - {book['year']} - {book['genre']} -(status)')
            else:
                print(f"No books found matching '{search_term}' is the {search_by} field")
